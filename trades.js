...
async function loadTrades() {
    ...
    try {
        ...
        // Build rosterDraftsMap for all seasons
        draftMap = await buildDraftMap(leagueMap);
...
function renderTrades() {
    ...
    filteredTrades.forEach((t, i) => {
        ...
        draftPicks.forEach(dp => {
            ...
            const teamName = rosterMap[originalRosterId]?.teamName || "Unknown";
            const userName = rosterMap[originalRosterId]?.displayName || "Unknown";

            let playerName = "Unknown";
            let pickNumber = dp.pick_no || "";
            
            let label = `Pick ${season} Round ${round} (${teamName})`;

            // Look up in draftMap
            if (
                draftMap[season] &&
                draftMap[season][round] &&
                draftMap[season][round][originalRosterId]
            ) {
                const pickInfo = draftMap[season][round][originalRosterId];
                console.log(pickInfo);
                playerName = pickInfo.player_name;
                pickNumber = pickInfo.round + "." + pickInfo.draft_slot;
                label = label + ` -> ${pickNumber} -> <em>${playerName}</em>`;
            }

            grouped[receiver] = grouped[receiver] || [];
            grouped[receiver].push(label);
        });
...
/**
 * Builds a complete draft map: season -> round -> roster_id -> { pick, player_id, player_name }
 */
async function buildDraftMap(leagueMap) {
    ...
    for (const [season, leagueId] of Object.entries(leagueMap)) {
        try {
            ...
            // Assume first draft is the main draft for season
            const draft = await fetch(`https://api.sleeper.app/v1/draft/${drafts[0].draft_id}`).then(r => r.json());
            ...
            picks.forEach(pick => {
                ...
                draftMap[season][round][rosterId] = {
                    absolute_pick: pick.pick_no, // absolute pick number
                    round: round,
            ...
            console.warn(`Failed to fetch draft data for league ${leagueId} season ${season}`, err);
        }
    }
